...
- **File upload — primary path**: prefer the browser surface's native file upload API (e.g. Playwright `setInputFiles()`). For Chrome-extension paths, use a localhost HTTP server fetch or chunked `sessionStorage` — never `localStorage`, which persists on the third-party ATS origin and retains PDF contents across sessions. Do NOT send a large inline base64 string to `Runtime.evaluate` (a 13 KB PDF produces a ~17 K-char literal that blocks the renderer and trips the CDP 30 s timeout). See `skills/apply/SKILL.md` Step 5. `mcp__claude-in-chrome__file_upload` requires base64 data, not filesystem paths (`paths` parameter is deprecated). Workable's `react-dropzone` does not re-render from DataTransfer injection — manual upload required.
